  ...
  try {
    if (platform === "android") {
      ...
      const token = await getToken(messaging, {
        vapidKey:
          "BLNEofFNYEcYaTRKWtRT3nj3xMwpUenDwX2Xqr3M3CT0hrLJt37iQTEiZcAhsE6WCt9wc16IMJTv39VrUgapI68",
      });
      ...
      toast.warn(
        "Push notifications not supported or PWA not installed on iOS."
      );
    }
  } catch (err) {
    console.error(" Error during push registration:", err);
    toast.error("Error during push registration");
    setToken(null);
  }
}
